...
export default function FloatingLines({
  ...
  // Memoize gradient computation to avoid recalculation
  ...
  // Memoize line count calculations
  const lineCountValues = useMemo(() => {
    const getLineCount = (waveType: 'top' | 'middle' | 'bottom'): number => {
      if (typeof lineCount === 'number') return lineCount;
      if (!enabledWaves.includes(waveType)) return 0;
      const index = enabledWaves.indexOf(waveType);
  ...
  // Memoize line distance calculations
  const lineDistanceValues = useMemo(() => {
    const getLineDistance = (waveType: 'top' | 'middle' | 'bottom'): number => {
      if (typeof lineDistance === 'number') return lineDistance;
      if (!enabledWaves.includes(waveType)) return 0.1;
      const index = enabledWaves.indexOf(waveType);
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      topLineCount: { value: lineCountValues.top },
      middleLineCount: { value: lineCountValues.middle },
      bottomLineCount: { value: lineCountValues.bottom },

      topLineDistance: { value: lineDistanceValues.top },
      middleLineDistance: { value: lineDistanceValues.middle },
      bottomLineDistance: { value: lineDistanceValues.bottom },
    ...
    // Only add event listeners if interactive is enabled
    ...
    const renderLoop = () => {
      ...
      // Only process mouse/parallax if enabled
      if (interactive) {
        currentMouseRef.current.lerp(targetMouseRef.current, mouseDamping);
        uniforms.iMouse.value.copy(currentMouseRef.current);
    ...
    return () => {
      ...
      if (interactive && handlePointerMove && handlePointerLeave) {
        renderer.domElement.removeEventListener('pointermove', handlePointerMove);
        renderer.domElement.removeEventListener('pointerleave', handlePointerLeave);
      }
  ...
  // Update uniforms when props change - no scene recreation needed
  useEffect(() => {
    ...
    uniforms.topLineCount.value = lineCountValues.top;
    uniforms.middleLineCount.value = lineCountValues.middle;
    uniforms.bottomLineCount.value = lineCountValues.bottom;

    uniforms.topLineDistance.value = lineDistanceValues.top;
    uniforms.middleLineDistance.value = lineDistanceValues.middle;
    uniforms.bottomLineDistance.value = lineDistanceValues.bottom;
