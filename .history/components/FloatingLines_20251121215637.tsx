...
// Ultra-optimized shader - removed all unused code
const fragmentShader = `
precision mediump float;
...
float wave(vec2 uv, float offset) {
  ...
  float timeOffset = offset + time * 0.2;
  float amp = sin(timeOffset) * 0.3;
  float y = sin(uv.x + x_offset + x_movement) * amp;
...
void main() {
  ...
  // Unrolled loop for maximum performance (lineCount=9)
  float fi, t;
  vec3 lineCol;
  ...
  t = 0.0;
  lineCol = lineColor;
  col += lineCol * wave(ruv + vec2(lineDistance * fi + wavePosition.x, wavePosition.y), 2.0 + 0.15 * fi);
...
type FloatingLinesProps = {
  ...
  mixBlendMode?: React.CSSProperties['mixBlendMode'];
  targetFPS?: number;
  enableLazyRendering?: boolean; // New: only render when visible
};
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (enableLazyRendering && typeof IntersectionObserver !== 'undefined') {
    ...
    const renderLoop = (currentTime: number) => {
      // Only render if visible (when lazy rendering is enabled)
      if (enableLazyRendering && !isVisibleRef.current) {
        raf = requestAnimationFrame(renderLoop);
        return;
      }
  ...
  }, [targetFPS, enableLazyRendering]); // Minimal dependencies
  ...
  useEffect(() => {
    ...
    uniforms.lineColor.value.copy(colorVector);
  }, [
    animationSpeed,
    lineDistance,
    wavePosition,
